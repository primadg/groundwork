...
class Smarty_Internal_Data {
    ...
    {            
        $oConfig = get_config();
	$oInstance = get_instance();

        if (is_array($tpl_var)) {
            foreach ($tpl_var as $_key => $_val) {
                if ($_key != '') {
		    if(in_array($key,$oConfig['reserved_words'] ))
		    {
			$oInstance->errors->set(UM_ERRORTYPE_MESSAGE, t('Can not redecalare of system variable name '.$key));
		    }
		    else
            ...
            if ($tpl_var != '') {
		if(in_array($tpl_var,$oConfig['reserved_words'] ))
		{
		    $oInstance->errors->set(UM_ERRORTYPE_MESSAGE, t('Can not redecalare of system variable name '.$tpl_var));
		}
		else
    ...
    /**
     * assigns a system Smarty variable
     *
     * @param array|string $tpl_var the template variable name(s)
     * @param mixed        $value   the value to assign
     * @param boolean      $nocache if true any output of this variable will be not cached
     ...
     * @return Smarty_Internal_Data current Smarty_Internal_Data (or Smarty or Smarty_Internal_Template) instance for chaining
     */
    public function sysassign($tpl_var, $value = null, $nocache = false)
    {
        if (is_array($tpl_var)) {
            foreach ($tpl_var as $_key => $_val) {
                if ($_key != '') {
